...
Function 	Patch-Template 
{
	...
	if ($RegExInstances.Length -eq 0) 
	{
		...
		[Int32[]]$RegExInstances = @(1)		# This ensures the RegExInstances is set to at least one and handles the param having no default value or not existing.
	}
	
	$FileContentRegExMatchInstance = 0		# Arrays start at zero, so incremented at the end of the (foreach) loop
